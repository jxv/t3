...
startMatch :: (Lobby m, Part m, MonadConc m) => StartRequest -> m StartResponse
...
randomMatch :: Part m => StartRequest -> m StartResponse
randomMatch startReq = do
  authenticate (_sreqCreds startReq)
  randomResponse startReq

registerUser :: (Part m, Storage m, MonadRandom m) => RegisterRequest -> m RegisterResponse
registerUser rreq = do
  let name@(UserName un) = _rreqName rreq
  userKey <- genUserKey
  users <- tryRegister name userKey
  storeUsers users
